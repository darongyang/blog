...
Q3：我想问有关冷热对象分离的问题，第一个是作者是如何识别冷热对象的，以及识别准确度有多少？为什么这个区间多分几份，优化效果就更好？冷热对象划分的依据是什么，为什么冷的对象要放入热的子集？

A3：（1）冷热对象的识别方式不是作者设计的核心，是直接利用的现有的一些算法RRIP，这些常用于处理器判断的算法。针对这个算法准确性没有做太多评估。（2）作者提到区间的过度划分会导致RRIP算法带来缓存不命中率的增加，因此不宜过多划分。（3）这里时间关系我没有深入介绍。这里有个缓存的概念。对对象而言，热是指频繁访问和读命中。对集合而言，热是指频繁的写入。频繁访问的对象更倾向于留在缓存中，而不期望被替换，因此要放入冷子集。
