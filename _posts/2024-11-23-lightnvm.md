...
description: "LightNVM is a FTL for Open-Channel SSD"
...
一周的时间，匆匆赶出来了LightNVM解析的Version 1。这是很粗糙的版本，但了解LightNVM已经不成问题。Version 2可能要等比较久以后再更新了，可能几个月后，自己工作被push有点紧了:-(。
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/posts/lightnvm/lightnvm.png" title="" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
    LightNVM在IO栈的位置
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/posts/lightnvm/io-path.png" title="" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
    LightNVM的IO Path
...
- pblk_down_rq
1.传入本次IO命令的第一个物理地址
2.根据该物理地址得到要访问的luns下标
3.调用__pblk_down_chunk将对应的luns上锁

- pblk_up_chunk
和上述获取操作相反，是释放lun的操作

- 上锁和释放时机
上锁时机：一个写请求到来的时候，分配物理地址的时候将对应的luns上锁
释放时机：一个写请求完成的时候，在end_io时候调用释放函数将对应的luns释放
...
- `inflight_flush_point`，当前有多少个等待flush的请求
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/posts/lightnvm/gc.png" title="" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
    pblk的GC流程示意
...
- `pblk_gc_line` / `pblk_gc_line_prepare_ws` 对一个line进行gc/回收
	...
	- `pblk_gc_line_ws` 根据gc_rq，唤醒读线程读取有效块并唤醒gc写线程写入有效块。
...
<div class="row mt-3">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/posts/lightnvm/write.png" title="" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
    pblk的写流程示意
...
- `pblk->writer_ts/pblk_write_ts/pblk_submit_write`，执行持久化落盘写入的写线程（和line管理、物理页等打交道，和`pblk_write_to_cache`通过write context构建桥梁）
	...
	- 分配写bio和创建设备写请求
	- `pblk_rb_read_to_bio` 将页面数据从ring buffer中拷贝到写bio中
